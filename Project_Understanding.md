...
- **Mutating methods**, each throwing a plain `Error` on a business-rule violation so the calling dialog can catch and display it inline:
  ...
  - `cancelTrip(tripId)` — only allowed while an *active* trip has just its start checkpoint (`checkpoints.length <= 1`); otherwise throws, directing the user to end the trip instead. Deletes the trip and its checkpoint(s) outright — this is for a trip that never really got underway.
  - `deleteTrip(tripId)` — only allowed for a *closed* trip; throws otherwise ("end or cancel it first"). Reuses the exact same two repository calls as `cancelTrip` (`tripCheckpointRepository.removeForTrip` then `tripRepository.remove`), dispatching a distinct `TRIP_DELETED` action (same state-cleanup shape as `TRIP_CANCELED`, kept separate for clarity — one is "abandon before it really started," the other is "remove a completed historical record"). This is the only way a closed trip's data is ever removed.
...
- **`AddEditEntryScreen.tsx`** — Shared form for both flows. Reads `route.params?.entryId`; if present, looks up the existing entry via `getEntryById` and pre-fills every field. Shows a small non-editable "🏍️/🚗 Logging for {vehicle name}" badge at the top — the entry's own vehicle when editing, the currently-selected vehicle when adding — since there's no per-entry vehicle picker in the form by design (the vehicle is chosen globally via `VehicleSelector`, not per entry). Client-side checks (all fields present, numeric fields actually numeric) run before calling `addEntry`/`updateEntry`; a thrown validation error (from `odometerValidation`, surfaced through the context) is caught and shown as inline red text above the submit button. Root element is a `KeyboardAvoidingView` (`behavior: 'padding'` on iOS, `'height'` on Android) wrapping the existing `ScrollView` (`keyboardShouldPersistTaps="handled"`) — added after real-device testing found the last two fields, Station and Notes, getting hidden behind the keyboard with no way to scroll to them; same root cause and same fix as the two trip dialogs (see `StartTripDialog.tsx`/`AddCheckpointDialog.tsx` below).
...
- **`AddCheckpointDialog.tsx`** — A `Modal` form shared by both "Add a stop" and "End trip", taking `{ visible, tripId, isEnding, onClose }`. `isEnding` drives the title ("Add a stop" vs "End trip"), whether liters/cost are optional or required, and whether submit calls `addCheckpoint` or `endTrip`. Same `DateTimeField` + odometer + location fields as `StartTripDialog`, plus liters/cost.
  ...
  Both dialogs' cards (`maxHeight: '85%'`) wrap their field list in a `KeyboardAvoidingView` (`behavior: 'padding'` on iOS, `'height'` on Android) + `ScrollView` (`keyboardShouldPersistTaps="handled"`), with the Cancel/Confirm button row kept *outside* the scroll area so it stays pinned at the bottom of the card. Found necessary on real-device Android testing: without it, the soft keyboard could push the last field(s) — Liters/Cost, on `AddCheckpointDialog` — out of the fixed-height card with nothing to scroll them back into view.
...
**A closed trip can be deleted outright; an active one can only be canceled, and only before it really gets underway.** These are two deliberately different operations, not one feature with two names: `cancelTrip` exists for an *active* trip abandoned right after starting (only its `start` checkpoint exists) and hard-deletes it as if it never happened; `deleteTrip` exists for a *closed* trip's historical record and was added specifically so old/test/unwanted trips can be cleared out. Neither can be used for the other case — `deleteTrip` throws on a non-closed trip, `cancelTrip` throws once a trip has more than its start checkpoint — so the UI (`TripDetailScreen`) only ever shows the one that's actually valid for the trip's current state. As with vehicles and fuel entries, there's still no bulk/cascade delete anywhere in the trips feature.
