...
- An *active* trip can only be canceled while it has just its start checkpoint (nothing else added yet) — same "can't destroy real recorded data" principle already used for vehicle deletion. Past that point, finish the trip by ending it. A *closed* trip, by contrast, can always be deleted outright (added after Phase 8's initial build, on user request — see below) since its record is already complete and final; there's no partial-data risk to guard against there.
...
**Phase 8 on-device fixes and follow-up (Android, Expo Go)**: three issues surfaced only by testing on a real device (none reproducible on `expo start --web`, since none involve safe-area insets, the soft keyboard, or a real request for the feature):
- `TripDetailScreen`'s `SafeAreaView` only reserved the *top* inset (`edges={['top']}`). Unlike the tab screens, this is a plain pushed stack screen with no tab bar underneath to absorb the space — so its bottom content (the "Cancel this trip" link, in particular) rendered flush against the phone's gesture/nav bar and was partly unreachable. Fixed by adding `'bottom'` to `edges`.
- `StartTripDialog` and `AddCheckpointDialog` (fixed-height `Modal` cards, `maxHeight: '85%'`) had no `KeyboardAvoidingView`/`ScrollView` — when the soft keyboard opened over the last one or two fields (Liters/Cost, on `AddCheckpointDialog`), those fields were pushed out of the fixed-height card with nothing to scroll them back into view. Fixed by wrapping both cards' field lists in `KeyboardAvoidingView` (`padding` on iOS, `height` on Android) + a `ScrollView` (`keyboardShouldPersistTaps="handled"`), with the Cancel/Confirm button row kept outside the scroll area so it stays pinned at the bottom of the card.
- Added, on request: a "Delete this trip" action on `TripDetailScreen`, shown only for a *closed* trip, confirmed via the same `ConfirmDialog` pattern used everywhere else in the app. `TripsContext` gained a `deleteTrip(tripId)` method (throws if the trip isn't closed) that reuses the existing `tripCheckpointRepository.removeForTrip`/`tripRepository.remove` calls already written for `cancelTrip`, dispatching a new `TRIP_DELETED` action with the same state-cleanup shape as `TRIP_CANCELED`. This is the only way a *closed* trip's data can ever be removed — there's still no bulk/cascade delete, and an *active* trip still can't be deleted this way (it must be ended or, while only its start checkpoint exists, canceled).

All three confirmed working on-device.

**Also fixed on-device**: `AddEditEntryScreen` (the regular fuel log's Add/Edit form) had the identical keyboard-overlap bug as the two trip dialogs above — its last two fields, Station and Notes, got hidden behind the soft keyboard with no way to scroll them into view, since its `ScrollView` had no `KeyboardAvoidingView` around it. Same fix applied: wrapped in `KeyboardAvoidingView` (`padding` on iOS, `height` on Android), plus `keyboardShouldPersistTaps="handled"` on the `ScrollView`. Confirmed working on-device.
